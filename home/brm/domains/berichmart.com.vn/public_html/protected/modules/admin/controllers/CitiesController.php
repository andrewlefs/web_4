...
class CitiesController extends Controller{
    ...
    public function actionIndex(){
        ...
        $count = City::model()->count($criteria); // dem so ban ghi theo dieu kien
        ...
        $data = City::model()->findAll($criteria); // mang chua ds cac ban ghi theo dieu kien
	...
	$this->performAjaxValidation($cities);
        if(isset($_POST['City'])){ //pr($_POST); die;            
            $data = $_POST['City'];
            $data['status']=1;
            $data['created']=date('Y-m-d');
            $data['modified']=date('Y-m-d');
            $cities->attributes = $data;      
            if($cities->save())// chi luu khi cac truong du lieu khong null deu co gia trị
            $this->redirect(array('index'));
        }
        $this->render("add",array('cities'=>$cities));//...
    }
    public function actionEdit($id = null){
        checkLogin($this);
        $cities = City::model()->findByPk($id);
        $this->performAjaxValidation($cities);
        if(isset($_POST['City'])){ //pr($_POST); die;
            $data = $_POST['City'];
        ...
        $this->render("edit", array('cities'=>$cities));
    }
    public function actionDelete($id=null){
        checkLogin($this);
         City::model()->findByPk($id)->delete();
         $this->redirect(array('index'));
    }
    public function actionUpdateStatus($id){
     ...
     public function actionView($id=null) {
        checkLogin($this);
        $cities = City::model()->findByPk($id); 
        $this->render("view", array ('cat'=>$cities));
    }
