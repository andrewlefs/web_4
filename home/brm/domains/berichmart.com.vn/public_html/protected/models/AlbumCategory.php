...
class AlbumCategory extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array('Parent'=>array(self::BELONGS_TO,'AlbumCategory','parent_id'),
		);
	}
	...
	public function search()
	{
        ...
        public function getCountAlbum(){ 
            $count = Yii::app()->db->createCommand('select count(*) from albums where album_category_id like "%,'.$this->id.',%"')->queryScalar();            
            return $count;
        }
        
        public function getCountVideo(){
            $albums = Album::model()->findAll('album_category_id like "%,'.$this->id.',%"') ;  
            $albums = CHtml::listData($albums, 'id', 'id');
            if(!empty($albums)){
                $count = Yii::app()->db->createCommand('select count(*) from videos where album_id in ('.  implode(',', $albums).')')->queryScalar();
                return $count;
            }
            else return 0;
        }
